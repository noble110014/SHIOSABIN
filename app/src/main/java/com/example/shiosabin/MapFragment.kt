...
class MapFragment : Fragment(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        // viewLifecycleOwner.lifecycleScope を使ってコルーチンを開始
    ...
    private fun addCircleAroundMarker(
        ...
        // Fragmentがアタッチされていることを確認
        if (isAdded) {
            googleMap.addCircle(
                CircleOptions()
                    .center(latlng)
                    .radius(_radius)
                    .fillColor(ContextCompat.getColor(requireContext(), rangeColor)) // requireContext()
            )
